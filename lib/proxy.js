...
module.exports = Base.extend(function(opts){
  ...
  if(opts.tlspassthru) {
    opts.tlspassthru.SNICallback = function(servername, done) {
      //todo: fetch additional certificate details. 
      var certopts = {
        commonName: servername, //farendcert.subject.CN || hinfo.hostname,
        days: 7,
        selfSigned: true
        //todo add ca details here.
      };
      require('pem').createCertificate(certopts, function(err, keys) {
        done(null, require('tls').createSecureContext({cert: keys.certificate, key: keys.serviceKey}));
      });
    };
    ...
    thisProxy._passthru = https.createServer(opts.tlspassthru, function(req, res) {
      ...
      //var counts = 0;
      //res.on('data', function() { console.log("real size", counts); counts += arguments[0].length; });
      //res.on('end finish', function() { console.log("real", counts); });
      handler(req, res);
    });
