...
export default class Proxy extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    let httpd = (opts.reverse && opts.tlsopts) ? https.createServer.bind(https, opts.tlsopts) : http.createServer;
    ...
    if(opts.tlspassthru) {
      ...
      opts.tlspassthru.SNICallback = (servername, done) => {
        if(gencerts[servername]) {
          done(null, gencerts[servername]);
          return;
        }
        ...
        if(opts.caroot) {
          certopts.serviceCertificate = opts.caroot.cert;
          certopts.serviceKey = opts.caroot.key
          certopts.serial = Date.now();
          certopts.selfSigned = false;

          /*if(opts.tlspassthru.ca && Array.isArray(opts.tlspassthru.ca))
            opts.tlspassthru.ca.push(opts.caroot.cert);
          else if(opts.tlspassthru.ca && !Array.isArray(opts.tlspassthru.ca))
            opts.tlspassthru.ca = [opts.tlspassthru.ca, opts.caroot.cert];*/
        }

        pem.createCertificate(certopts, (err, keys) => {
          self.emit('log', {
            level: 'debug'
            ,message: 'Created certificate for domain ' + servername      
          });
          let sc = {cert: keys.certificate, key: keys.serviceKey}
          if(opts.caroot)
            sc.ca = [opts.caroot.cert];
          let ncert = tls.createSecureContext(sc);
          gencerts[ncert];
          done(err, ncert);
        })
      }
      ...
      this._tlspassthru.on('clientError', function() {
        console.log(arguments[0]);
      })
      this._tlspassthru.listen(function() {
        self.emit('log', {
          level: 'debug'
          ,message: 'tlspassthru is listening on port ' + self._tlspassthru.address().port
        })
      });
    }
