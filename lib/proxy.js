...
module.exports = Base.extend(function(opts){
  ...
  // --------------------------------------------------
  var handler = function(inReq, outResp){
  ...
  thisProxy._server = httpd(handler);
  ...
  thisProxy._server.on('connect', function(inReq, socket, head) {
    ...
    var freq = require('tls').connect(hinfo.port, hinfo.hostname, {rejectUnauthorized: false}, function() {
      ...
      require('pem').createCertificate(certopts, function(err, keys) {
        ...
        if(!!!thisProxy._passthru) {
          freq.end();
          socket.end('');
          return;
        }
        thisProxy._passthru.addContext(hinfo.hostname, {cert: keys.certificate, key: keys.serviceKey});
