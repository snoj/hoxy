...
module.exports = Base.extend(function(opts){
  ...
  var httpd = (this._tls) ? https.createServer.bind(https, this._tls) : http.createServer;
  thisProxy._server = httpd(function(inReq, outResp){
  ...
  thisProxy._server.on('connect', function(inReq, socket, head) {
    var hinfo = require('url').parse('http://' + inReq.url);
    var freq = require('tls').connect(hinfo.port, hinfo.hostname, {rejectUnauthorized: false}, function() {
      var farendcert = freq.getPeerCertificate(true);
      var certopts = {
        commonName: farendcert.subject.CN || hinfo.hostname,
        days: 7,
        selfSigned: true
      };

      if(farendcert.subjectaltname) {
        certopts.altNames = farendcert.subjectaltname.split(",").map(function(v) {
          return v.replace("DNS:", "").trim();
        });
      }

      require('pem').createCertificate(certopts, function(err, keys) {
        var tmphttp = https.createServer({key: keys.serviceKey, cert: keys.certificate});
        tmphttp.on('request', function(req, res) {
        ...
        tmphttp.on('listening', function() {
          var t = tmphttp.address();
          var nearreq = require('net').connect(t.port, '127.0.0.1', function() {
